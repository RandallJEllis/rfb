...
def anno_protein_set(proteins):
    ...
    prot_annot = pd.read_csv(os.path.join(script_dir,
                             '../tidy_data/proteomics/coding143.tsv',
                                          sep='\t'))
...
def pull_proteins(path, outcome, nf, iteration):
    ...
    sub = df[(df.outcome == f'{outcome}-0.0') & (df.n_features == nf) &
             (df.iteration == iteration)].iloc[0]
...
def main():
    ...
    outcome_top_rows = outcome_top_rows.sort_values(by=['best_f1'],
                                                    ascending=False)
    ...
    for i in range(outcome_top_rows.shape[0]):
        print(i)
        dfdf = pull_proteins('../tidy_data/bootstrap/individual_results/',
