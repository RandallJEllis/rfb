        ...
        for model in "${models[@]}"; do
            for age_cutoff in "${age_cutoffs[@]}"; do
                for region_index in {0..0}; do
                    ...
                    sbatch --partition="$partition" --time="$time" --export=experiment="$experiment",metric="$metric",model="$model",age_cutoff="$age_cutoff",region_index="$region_index" sh_ml_experiments.sh
