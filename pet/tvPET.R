...
model1 <- models_list$demographics$fold_2
model2 <- models_list$centiloids_demographics$fold_2
df <- train_df_l$fold_2_demographics
all.equal(train_df_l$fold_2_demographics,
          train_df_l$fold_2_centiloids_demographics) # must be TRUE
...
ggsave("../tidy_data/risk_reclassification_heatmap.pdf",
...
write.csv(results_table, "../tidy_data/auc_comparison_results.csv",
          row.names = FALSE)
...
print(hist_pvalues)
ggsave("../tidy_data/pvalue_histogram_pTau217_Demo_Lancet_vs_Demo_Lancet.pdf",
       plot = hist_pvalues,
       width = 8,
       height = 6,
       dpi = 300)
...
print(auc_plot)
ggsave(
  "../tidy_data/auc_differences_boxplot.pdf",
  plot = auc_plot,
  width = 8,
  height = 6,
  dpi = 300
...
model_labels <- c(
  ...
  "centiloids_demographics" = "Centiloids+Demo",
  "centiloids_demographics_no_apoe" = "Centiloids+Demo (-APOE)"
