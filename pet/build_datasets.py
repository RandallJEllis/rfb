...
def find_first_or_last_visit(
    df_list, id_col="RID", examdate_col="EXAMDATE", first_or_last="first"
):
    """
    ...
    if first_or_last == "first":
        df_list = [
            df.groupby(id_col)[examdate_col].first().reset_index() for df in df_list
        ]
    elif first_or_last == "last":
        df_list = [
            df.groupby(id_col)[examdate_col].last().reset_index() for df in df_list
        ]
    else:
        raise ValueError("first_or_last must be 'first' or 'last'")

    df_list = [
        df.rename({examdate_col: f"{first_or_last}_{examdate_col}"}, axis=1)
        for df in df_list
    ]
    ...
    if first_or_last == "first":
        merged[examdate_col] = merged.apply(
            lambda row: (
                min(
                    row[f"{first_or_last}_{examdate_col}_x"],
                    row[f"{first_or_last}_{examdate_col}_y"],
                )
                if pd.notna(row[f"{first_or_last}_{examdate_col}_x"])
                and pd.notna(row[f"{first_or_last}_{examdate_col}_y"])
                else (
                    row[f"{first_or_last}_{examdate_col}_x"]
                    if pd.notna(row[f"{first_or_last}_{examdate_col}_x"])
                    else row[f"{first_or_last}_{examdate_col}_y"]
                )
            ),
            axis=1,
        )
...
def map_time_from_baseline(
    df, earliest_baseline, id_col="RID", examdate_col="EXAMDATE"
):
    """
    ...
    """
    df["visit_to_days"] = df.apply(
        lambda row: (
            row[examdate_col]
            - earliest_baseline[earliest_baseline[id_col] == row[id_col]][
                examdate_col
            ].values[0]
        ).days,
        axis=1,
...
def process_oasis(
    oasis_file_path, oasis_diagnoses_file_path, oasis_demo_file_path, ad_outcome=False
):
    ...
    cols = ["ID", "tracer", "day"] + [
        col for col in df if col not in ["ID", "tracer", "day"]
    ]
    df = df[cols]
    ...
    diagnoses[["ID", "phase", "day"]] = diagnoses["OASIS_session_label"].str.split(
        "_", expand=True
    )

    # Move the new columns to be the first columns
    cols = ["ID", "phase", "day"] + [
        col for col in diagnoses if col not in ["ID", "phase", "day"]
    ]
    diagnoses = diagnoses[cols]
    diagnoses["day"] = diagnoses["day"].str.replace("d", "").astype(int)
    diagnoses = diagnoses.sort_values(by=["ID", "day"])

    # sum the dx columns that are relevant to dementia and its subtypes
    diagnoses["dx_sum"] = diagnoses.iloc[:, 8:].sum(axis=1)

    # if ad_outcome is true, use AD as the outcome
    # cases are those with AD
    # controls are those without AD or any other dx
    if ad_outcome:
        # cases are those with AD
        cases_dx = diagnoses[diagnoses["AD"] == 1]
    else:
        # cases are those with any dx
        cases_dx = diagnoses[diagnoses["dx_sum"] > 0]

    # find earliest diagnosis for each case
    case_dx_first = cases_dx.groupby("ID").first().reset_index()
    cases_dx_ids = cases_dx.ID.unique()
    pet_pts = df.ID.unique()
    cases_pet = np.intersect1d(cases_dx_ids, pet_pts)

    latest_date = find_first_or_last_visit(
        [df, diagnoses], id_col="ID", examdate_col="day", first_or_last="last"
    )
    ...
    for c in case_dx_first.ID.unique():
        ...
        if c_pet_before_dx.shape[0] == 0:
            ...
            c_pet.loc[:, "time_to_event"] = c_first_dx[c_first_dx["ID"] == c][
                "day"
            ].values[0]
    ...
    # controls are those without AD or any other dx
    all_case_ids = diagnoses[diagnoses["dx_sum"] > 0]["ID"].unique()
    pet_control_df = df[~df["ID"].isin(all_case_ids)]
    ...
    pet_control_df = pet_control_df.merge(
        pd.DataFrame({"ID": pet_control_df.ID.unique(), "time_to_event": control_t2e}),
        on="ID",
        how="left",
    )
    ...
    pet_df = pet_df.merge(
        demo.loc[:, ["OASISID", "AgeatEntry", "GENDER", "EDUC", "SES", "race", "APOE"]],
        left_on="ID",
        right_on="OASISID",
        how="left",
    )
...
def process_nacc(nacc_file_path, nacc_centiloids_file_path, ad_outcome=False):
    # Load raw NACC data
    ...
    latest_date = map_time_from_baseline(
        latest_date, earliest_baseline, id_col="NACCID", examdate_col="VISITDATE"
    )

    # Map time from baseline for both datasets
    centiloids = map_time_from_baseline(
        centiloids, earliest_baseline, id_col="NACCID", examdate_col="VISITDATE"
    )
    df = map_time_from_baseline(
        df, earliest_baseline, id_col="NACCID", examdate_col="VISITDATE"
    )
    ...
    case_dx_first = case_dx.sort_values(by=["NACCID", "VISITDATE"]).drop_duplicates(
        subset=["NACCID"], keep="first"
    )
    ...
    for c in case_dx_first.NACCID.unique():
        ...
        if c_pet_before_dx.shape[0] == 0:
            ...
            c_pet = c_pet.merge(
                c_first_dx[["NACCID", "visit_to_days"]], on="NACCID", how="left"
            )
    ...
    pet_control_df = pet_control_df[
        pet_control_df.NACCID.isin(df[df.NACCNORM == 1].NACCID.unique())
    ]

    # Add time-to-event information for controls (time to last visit)
    pet_control_df = pet_control_df.merge(
        latest_date[["NACCID", "visit_to_days"]], on="NACCID", how="left"
    )
    ...
    pet_df = pet_df.merge(
        df.loc[
            :, ["NACCID", "BIRTHDAY", "SEX", "NACCAPOE", "NACCNE4S", "EDUC"]
        ].drop_duplicates(),
        on="NACCID",
        how="left",
    )
...
def process_habs(
    habs_file_path, habs_centiloids_file_path, habs_demo_file_path, habs_pib_file_path
):
    # Load clinical diagnosis data
    dx = pd.read_csv(habs_file_path)
    dx["NP_SessionDate"] = pd.to_datetime(dx["NP_SessionDate"])
    dx.rename(
        {"NP_SessionDate": "EXAMDATE", "SubjIDshort": "RID"}, axis=1, inplace=True
    )
    ...
    # Calculate baseline visits and time from baseline
    earliest_baseline = find_first_or_last_visit(
        [centiloids, dx], first_or_last="first"
    )
    ...
    for c in case_dx_first.RID.unique():
        ...
        if c_pet_before_dx.shape[0] == 0:
            ...
            # Add time-to-event information
            c_pet = c_pet.merge(
                c_first_dx[["RID", "visit_to_days"]], on="RID", how="left"
            )
            c_pet.rename({"visit_to_days_x": "visit_to_days"}, axis=1, inplace=True)
            c_pet.rename({"visit_to_days_y": "time_to_event"}, axis=1, inplace=True)
            pet_case_df.append(c_pet)
    ...
    # Add time-to-event information for controls (time to last visit)
    pet_control_df = pet_control_df.merge(
        latest_date[["RID", "visit_to_days"]], on="RID", how="left"
    )
    pet_control_df.rename({"visit_to_days_x": "visit_to_days"}, axis=1, inplace=True)
    pet_control_df.rename({"visit_to_days_y": "time_to_event"}, axis=1, inplace=True)
    ...
    pib.rename(
        {"SubjIDshort": "RID", "PIB_SessionDate": "EXAMDATE"}, axis=1, inplace=True
    )
    ...
    pet_df = pet_df.merge(
        pib[["RID", "EXAMDATE", "PIB_Age"]], on=["RID", "EXAMDATE"], how="left"
    )
    return pet_df
...
def process_adni(
    ...
    # Identify cases
    if ad_outcome:
        case_dx = dx[(dx.DXAD == 1) | (dx.DXMDUE == 1) | (dx.DXAPP == 1)]
    ...
    pet_control_df = df[~df.RID.isin(all_exclusion_ids)]
...
def process_aibl(
    ...
    # Calculate baseline visits and time from baseline
    earliest_baseline = find_first_or_last_visit(
        [cdr, centiloids], first_or_last="first"
    )
    ...
    dx = dx.merge(
        cdr[["RID", "VISCODE", "EXAMDATE"]], on=["RID", "VISCODE"], how="left"
    )
    dx.sort_values(by=["RID", "EXAMDATE"], inplace=True)
    ...
    if ad_outcome:
        ...
        case_dx = dx[(dx.DXCURREN > 2) | (dx.DXAD == 1) | (dx.DXOTHDEM == 1)]
    case_dx_first = case_dx.drop_duplicates(subset=["RID"], keep="first")
    ...
    # Process control subjects (those who never developed dementia)
    all_case_ids = dx[
        (dx.DXCURREN > 2) | (dx.DXAD == 1) | (dx.DXOTHDEM == 1) | (dx.DXPARK == 1)
    ]
    pet_control_df = centiloids[~centiloids.RID.isin(all_case_ids)]
    ...
    adni_demo = pd.read_csv(adni_demo_file_path)
    demo = demo.merge(
        adni_demo.loc[:, ["RID", "PTEDUCAT"]].drop_duplicates(),
        on="RID",
        how="left",
    )
    pet_df = pet_df.merge(
...
def create_stratified_folds(data, n_splits=5, n_bins=4, random_state=42):
    ...
    assert (
        bids["fold"].min() >= 0 and bids["fold"].max() < n_splits
...
def process_fold(data, fold_assignments, fold):
    ...
    train_set["centiloids_z"] = (
        train_set.centiloids - training_centiloids_mean
    ) / training_centiloids_std
    val_set["centiloids_z"] = (
        val_set.centiloids - training_centiloids_mean
    ) / training_centiloids_std
    ...
    # z-score education
    training_education_mean = train_set.education.mean()
    training_education_std = train_set.education.std()

    train_set["education_z"] = (
        train_set.education - training_education_mean
    ) / training_education_std
    val_set["education_z"] = (
        val_set.education - training_education_mean
    ) / training_education_std
...
def make_cv_folds(output_path, ad_outcome=False):
    ...
    adni = pd.read_parquet(output_path + ad_outcome_path + "adni.parquet")
    adni = adni.loc[
        :,
        [
            "RID",
    ...
    aibl = pd.read_parquet(output_path + ad_outcome_path + "aibl.parquet")
    # keep only subjects with RID NOT contained in adni
    aibl = aibl[~aibl.RID.isin(adni.id)]
    aibl = aibl.loc[
        :,
        [
            "RID",
    ...
    if not NO_HABS:
        habs = pd.read_parquet(output_path + ad_outcome_path + "habs.parquet")
        habs = habs.loc[
            :,
            [
                "RID",
    ...
    nacc = pd.read_parquet(output_path + ad_outcome_path + "nacc.parquet")
    nacc = nacc.loc[
        :,
        [
            "NACCID",
    ...
    oasis = pd.read_parquet(output_path + ad_outcome_path + "oasis.parquet")
    oasis = oasis.loc[
        :,
        [
            "ID",
    ...
    if not NO_HABS:
        datasets = [adni, aibl, habs, nacc, oasis]
    else:
        datasets = [adni, aibl, nacc, oasis]
    data = pd.concat(datasets)

    #### Create folds for each cohort
    for i, val_set in enumerate(datasets):
        train_set = [ds for j, ds in enumerate(datasets) if j != i]
        ...
        if ad_outcome:
            train_set.to_parquet(
                output_path + ad_outcome_path + f"train_{i}_test_by_cohort.parquet"
            )
            val_set.to_parquet(
                output_path + ad_outcome_path + f"val_{i}_test_by_cohort.parquet"
            )
        else:
...
if __name__ == "__main__":
    ...
    nacc_centiloids_file_path = (
        main_path
        + "NACC/csv/raw/investigator_scan_pet_nacc65/investigator_scan_amyloidpetgaain_nacc65.csv"
    )

    habs_file_path = (
        main_path + "HABS/csv/raw/ClinicalMeasures_HABS_DataRelease_2.0.csv"
    )
    habs_centiloids_file_path = main_path + "HABS/csv/processed/HABS_CENTILOIDS.csv"
    habs_demo_file_path = (
        main_path + "HABS/csv/raw/Demographics_HABS_DataRelease_2.0.csv"
    )
    habs_pib_file_path = main_path + "HABS/csv/raw/PIB_FS6_DVR_HABS_DataRelease_2.0.csv"

    adni_pet_file_path = (
        main_path
        + "ALL_ADNI_LONIdownload/csv/raw/PET_UCBerkeley/UCBERKELEY_AMY_6MM_17Oct2023.csv"
    )
    adni_dx_file_path = (
        main_path + "ALL_ADNI_LONIdownload/csv/raw/DXSUM_PDXCONV_ADNIALLcsv.csv"
    )
    adni_apoe_file_path = (
        main_path + "ALL_ADNI_LONIdownload/csv/raw/APOERES_01Jun2023.csv"
    )
    adni_demo_file_path = (
        main_path + "ALL_ADNI_LONIdownload/csv/raw/PTDEMOG_29Nov2023.csv"
    )

    aibl_cdr_file_path = main_path + "AIBL/csv/raw/aibl_cdr_01-Jun-2018.csv"
    aibl_centiloids_file_path = (
        main_path + "AIBL/csv/processed/AIBL_CENTILOIDS_varuna.csv"
    )
    ...
    # Process OASIS Data #
    #######################
    oasis_df = process_oasis(
        oasis_file_path, oasis_diagnoses_file_path, oasis_demo_file_path
    )
    oasis_df.to_parquet(output_path + "oasis.parquet")

    oasis_df_ad_outcome = process_oasis(
        oasis_file_path,
        oasis_diagnoses_file_path,
        oasis_demo_file_path,
        ad_outcome=True,
    )
    oasis_df_ad_outcome.to_parquet(output_path + "ad_outcome/oasis.parquet")
    ...
    nacc_df_ad_outcome = process_nacc(
        nacc_file_path, nacc_centiloids_file_path, ad_outcome=True
    )
    nacc_df_ad_outcome.to_parquet(output_path + "ad_outcome/nacc.parquet")

    #######################
    # Process HABS Data #
    #######################
    habs_df = process_habs(
        habs_file_path,
        habs_centiloids_file_path,
        habs_demo_file_path,
        habs_pib_file_path,
    )
    habs_df.to_parquet(output_path + "habs.parquet")

    # No HABS with AD outcome because it isn't defined (only 6 Demented cases anyway)

    #######################
    # Process ADNI Data #
    #######################
    adni_df = process_adni(
        adni_pet_file_path, adni_dx_file_path, adni_apoe_file_path, adni_demo_file_path
    )
    adni_df.to_parquet(output_path + "adni.parquet")

    adni_df_ad_outcome = process_adni(
        adni_pet_file_path,
        adni_dx_file_path,
        adni_apoe_file_path,
        adni_demo_file_path,
    ...
    adni_df_ad_outcome.to_parquet(output_path + "ad_outcome/adni.parquet")

    #######################
    # Process AIBL Data #
    #######################
    aibl_df = process_aibl(
        aibl_cdr_file_path,
        aibl_centiloids_file_path,
        aibl_dx_file_path,
        aibl_demo_file_path,
    ...
    aibl_df_ad_outcome = process_aibl(
        aibl_cdr_file_path,
        aibl_centiloids_file_path,
        aibl_dx_file_path,
        aibl_demo_file_path,
    ...
    aibl_df_ad_outcome.to_parquet(output_path + "ad_outcome/aibl.parquet")
