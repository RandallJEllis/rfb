...
preprocess_df <- function(df, pacc_col, pacc, habits, psychwell, vitals, centiloids){
    ...
    df <- df %>%
    left_join(
      pacc %>% mutate(pacc_time = as.numeric(as.character(VISCODE))),
      by = c("id", "VISCODE")
    )

    # 3. Join with other datasets (REMOVE the VISCODE == x filters!)
    df <- df %>%
        left_join(habits, by = c("id", "VISCODE")) %>%
        left_join(psychwell, by = c("id", "VISCODE")) %>%
        left_join(vitals, by = c("id", "VISCODE")) %>%
        left_join(centiloids, by = c("id", "VISCODE"))

    # 4. Remove missing pacc values and covariates
    df <- df %>% filter(!is.na(.data[[pacc_col]]))
    # df <- df %>% drop_na(ptau, centiloid, apoe, age, sex)

    # 5. Standardize column names and factor levels
    colnames(df) <- str_to_lower(colnames(df))
    df <- df %>% rename(bmi = bmi1, bp = sbp, smoking = smoking4cat)

    df <- df %>%
        mutate(apoe = factor(apoe, levels = c("E3/E3", "E2/E3", "E2/E2", "E3/E4", "E4/E4", "E2/E4")))
...
pacc <- pacc %>% rename(id = BID)

habits <- read_parquet(paste0(load_path, "habits.parquet"))
habits <- habits %>% rename(id = BID)
habits$SUBUSE <- as.factor(habits$SUBUSE)
psychwell <- read_parquet(paste0(load_path, "psychwell.parquet"))
psychwell <- psychwell %>% rename(id = BID)
vitals <- read_parquet(paste0(load_path, "vitals.parquet"))
vitals <- vitals %>% rename(id = BID)
centiloids <- read_parquet(paste0(load_path, "centiloids.parquet"))
centiloids <- centiloids %>% rename(id = BID)
...
val_df_l <- qs::qread('../../tidy_data/A4/val_df_l.qs')


ddd <- train_df %>% left_join(habits, by = c("id", "VISCODE")) %>% 
left_join(psychwell, by = c("id", "VISCODE")) %>% 
left_join(vitals, by = c("id", "VISCODE")) %>%
left_join(centiloids, by = c("id", "VISCODE"))
