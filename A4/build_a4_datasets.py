...
def vectorized_age_calculation(data):
    ...
    age_increments = np.where(
        same_bid_mask,
        (
            result["COLLECTION_DATE_DAYS_CONSENT"]
            - result["COLLECTION_DATE_DAYS_CONSENT"].shift(1)
        )
        / 365.25,
        0,
    )
...
def time_to_consecutive_CDR(sub_df):
    ...
    if consecutive_highs.any():
        ...
        index_of_second = (
            consecutive_highs.idxmax()
        )  # idxmax gives the first True index
...
def merge_demo(df):
    ...
    """
    demo = pd.read_csv(
        f"../../raw_data/A4_oct302024/clinical/Derived Data/SUBJINFO.csv"
    )
    demo = demo[demo.BID.isin(df.BID.unique())]
    df = df.merge(
        demo.loc[
            :, ["BID", "AGEYR", "SEX", "RACE", "EDCCNTU", "ETHNIC", "APOEGN", "TX"]
        ],
        on="BID",
        how="left",
    )
...
def phenotype_cdr(df, cdr, sv):
    ...
    sv_controls = sv[
        (sv["BID"].isin(controls.BID)) & (sv.COLLECTION_DATE_DAYS_CONSENT.notna())
    ]
    control_t2e = (
        sv_controls.groupby("BID")
        .COLLECTION_DATE_DAYS_CONSENT.max()
        .reset_index(name="time_to_event")
        .sort_values(by="time_to_event")
    )

    # Update control time to event
    controls = controls.drop(columns=["time_to_event"]).merge(
        control_t2e, on="BID", how="left"
    )
    ...
    final_visit = sv[
        (sv["BID"].isin(df.BID)) & (sv.COLLECTION_DATE_DAYS_CONSENT.notna())
    ]
    final_visit = (
        final_visit.groupby("BID")
        .COLLECTION_DATE_DAYS_CONSENT.max()
        .reset_index(name="final_visit")
        .sort_values(by="final_visit")
    )
    ...
    print(
        f"{len(cases)} patients have a CDR of 0.5 or higher for two consecutive visits"
    )

    cases_with_ptau217 = np.intersect1d(
        result[result.time_to_event.notna()].BID.unique(), df.BID.unique()
    )
    print(
        f"{len(cases_with_ptau217)}/{len(cases)} patients have both CDR data and pTau217 data"
    )

    return df, cases_bid


def fix_time_dependent_labels(
    df,
    cases_bid,
    id_col="BID",
    time_col="stop",
    ...
    for subject_id in cases_bid:
        ...
        if pd.isna(first_case_idx):
            ...
            processed_data.append(subject_data.loc[:first_case_idx])
...
def merge_e2_carriers(df):
    ...
    df.APOEGN = df.APOEGN.replace({"E2/E3": "E2_carrier", "E2/E2": "E2_carrier"})
    return df
...
def get_ptau():
    ...
    ptau217 = load_ptau_data(
        f"../../raw_data/A4_oct302024/clinical/External Data/biomarker_pTau217.csv"
    )
    ...
    sv.rename(
        columns={
            "VISITCD": "VISCODE",
            "SVUSEDTC_DAYS_CONSENT": "COLLECTION_DATE_DAYS_CONSENT",
        },
        inplace=True,
    )
    ...
    cdr = cdr.merge(
        sv[["BID", "VISCODE", "COLLECTION_DATE_DAYS_CONSENT"]], on=["BID", "VISCODE"]
    )
    ...
    data.loc[
        (data["BID"] == "B69890108") & (data.COLLECTION_DATE_DAYS_T0 == 84),
        "COLLECTION_DATE_DAYS_CONSENT",
    ] = 2591
    data.loc[
        (data["BID"] == "B69890108") & (data.COLLECTION_DATE_DAYS_T0 == 84),
        "COLLECTION_DATE_DAYS_T0",
    ] = 2450
    ...
    data["time_to_event"] = np.where(
        (~data["BID"].isin(cases_bid))
        & (data["COLLECTION_DATE_DAYS_CONSENT"] > data["time_to_event"]),
...
def get_centiloids(data, sv):
    ...
    subjinfo = pd.read_csv(
        "../../raw_data/A4_oct302024/clinical/Derived Data/SUBJINFO.csv"
    )
    ...
    imaging_pet_va = pd.read_csv(
        "../../raw_data/A4_oct302024/clinical/External Data/imaging_PET_VA.csv"
    )
    imaging_pet_va = imaging_pet_va[imaging_pet_va.BID.isin(data.id)]

    # Merge CDR and visit data
    imaging_pet_va = imaging_pet_va.merge(
        sv[["BID", "VISCODE", "COLLECTION_DATE_DAYS_CONSENT", "SVUSEDTC_DAYS_T0"]],
        on=["BID", "VISCODE"],
    )
    subjinfo = subjinfo.merge(imaging_pet_va, on="BID", how="left")

    subjinfo["COLLECTION_YEARS_CONSENT"] = (
        subjinfo["COLLECTION_DATE_DAYS_CONSENT"] / 365.25
    )
...
def add_collection_date(newdata, sv):
    newdata = newdata.merge(
        sv[["BID", "VISCODE", "COLLECTION_DATE_DAYS_CONSENT"]],
        on=["BID", "VISCODE"],
        how="left",
    )
    return newdata
...
def main():
    ...
    data.rename(
        columns={
            "BID": "id",
            "COLLECTION_DATE_DAYS_CONSENT": "visit_to_days",
            "AGEYR": "age",
            "EDCCNTU": "educ",
            "ORRES": "ptau",
        },
    ...
    data_positive = data_positive.merge(
        centiloids_positive[["BID", "AMYLCENT", "overall_score"]],
        left_on="id",
        right_on="BID",
        how="left",
    )
    ...
    # save centiloids
    centiloids_positive.to_parquet(
        "../../tidy_data/A4/amyloid_positive/centiloids.parquet"
    )
    ...
    psychwell = pd.read_csv(
        f"../../raw_data/A4_oct302024/clinical/Raw Data/psychwell.csv"
    )
    ...
    phyneuro = pd.read_csv(
        f"../../raw_data/A4_oct302024/clinical/Raw Data/phyneuro.csv"
    )
