...
get_model_labels <- function(model_names) {
  ...
  return(model_labels[model_names])
}
...
print_model_stats <- function(models_list, var_string, return_table = FALSE) {
  ...
  if (return_table) {
    # calculate mean and sd of coefs and range of pvals
    # Show mean and SD as mean (SD) and range as min-max
    # round all values to 3 decimal places but keep e-notation for p-values
    coefs_summary <- data.frame(model = names(coefs),
                                coef = unlist(coefs),
                                pval = unlist(pvals)) %>%
      group_by(model) %>%
      summarise(coef = paste0(round(mean(coef), 3), " (", round(sd(coef), 3), ")"),
                pval = paste0(format(min(pval), scientific = TRUE), "-", format(max(pval), scientific = TRUE)))

    # sort by coef in ascending order
    coefs_summary <- coefs_summary %>%
      arrange(coef)

    # clean up model names
    model_labels <- get_model_labels(names(models_list))
    coefs_summary$model <- model_labels[coefs_summary$model]

    # clean up column names
    colnames(coefs_summary) <- c("Model", "Coef", "P-value")

    return(coefs_summary)
  }
}
