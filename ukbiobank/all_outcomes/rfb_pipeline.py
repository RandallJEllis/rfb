...
path = "../../tidy_data/UKBiobank/all_outcomes"

# Create the argument parser
parser = argparse.ArgumentParser(
    description="Random feature baselines pipeline with parallelization"
)
...
print("Running job number ", outcome_idx)
...
def main():
    ...
    def bootstrap(
        true_labels, proba_predict, n_bootstrap, best_threshold, bs_w_replace=False
    ):
        def append_results():
            yhat = proba_predict[bs_index]
            y_bs = true_labels.values[bs_index]

            auroc = roc_auc_score(y_bs, yhat)
            ...
            tn, fp, fn, tp = confusion_matrix(y_bs, test_pred).ravel()
            acc = accuracy_score(y_bs, test_pred)
            bal_acc = balanced_accuracy_score(y_bs, test_pred)
            prfs = precision_recall_fscore_support(y_bs, test_pred)
        ...
        if bs_w_replace is False:
            ...
            bootstraps = np.array(
                [
                    np.random.choice(
                        range(len(true_labels)), len(true_labels), replace=True
                    )
    ...
    categ_enc = pd.DataFrame(
        enc.transform(df.loc[:, ["31-0.0", "54-0.0"]]).toarray(),
        columns=enc.get_feature_names_out(["31-0.0", "54-0.0"]),
    )
    ...
    # for i, oc in zip(list(range(outcome_idx)), outcome_subset):
    print("outcome:", oc)
    ...
    for nfeat in [len(proteins)]:  #   # 5, 50, 100, 500, 1000,
        ...
        if nfeat < len(proteins):
            for i in range(n_subset):
                ...
                # fit to training set
                # clf = HistGradientBoostingClassifier(class_weight="balanced").fit(
                #     X.iloc[train_idx], y_train
                # )

                n_pos = sum(y_train)
                n_neg = len(y_train) - n_pos
                clf = lgb.LGBMClassifier(scale_pos_weight=n_neg / n_pos).fit(
                    X.iloc[train_idx], y_train
                )

                # save model
                os.makedirs(f"{path}/bootstrap/models/nfeat_{nfeat}", exist_ok=True)
                joblib.dump(
                    clf,
                    f"{path}/bootstrap/models/nfeat_{nfeat}/{oc}_model_{outcome_idx}_{i}.pkl",
                )

                best_threshold = ml.pick_threshold(
                    y_train, clf.predict_proba(X.iloc[train_idx])[:, 1]
                )
            ...
            joblib.dump(
                clf,
                f"{path}/bootstrap/models/nfeat_{nfeat}/{oc}_model_{outcome_idx}.pkl",
            )
    ...
    data = {
        ...
        "auroc": auroc_l,
        "avg_prec": ap_l,
        "best_thresh": best_threshold_l,
        # "best_f1": best_fscore_l,
        "accuracy": accuracy_l,
        "balanced_acc": balanced_accuracy_l,
        "prec_neg": prec_n,
        "prec_pos": prec_p,
        "rec_neg": rec_n,
        "rec_pos": rec_p,
    ...
    if nfeat == len(proteins):
        results.drop(columns=["proteins"], inplace=True)

    if bs_w_replace == True:
        results.to_parquet(
            f"{path}/bootstrap/results_{oc}_bsWreplace_{outcome_idx}.parquet"
        )
    else:
        results.to_parquet(f"{path}/bootstrap/results_{oc}_{outcome_idx}.parquet")
