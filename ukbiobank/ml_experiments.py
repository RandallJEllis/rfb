...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run AutoML on chosen feature sets")
    ...
    parser.add_argument("--model", type=str, default="lgbm", help="options: lgbm, lrl1")
    parser.add_argument("--age_cutoff", type=int, default=None, help="age cutoff")
    parser.add_argument("--region_index", type=int, required=True, help="region index")
    parser.add_argument(
        "--predict_alzheimers_only",
        type=int,
        default=0,
...
def get_dir_path(
    data_modality, experiment, metric, model, alzheimers_only=False, survival=False
):
    """
    ...
    """
    if alzheimers_only:
        base_path = "../results/UKBiobank/alzheimers"
    else:
        base_path = "../results/UKBiobank/dementia"
    ...
    if "fs_" in experiment:  # running a feature selection experiment
        ...
        original_results_directory_path = (
            f"{base_path}/{data_modality}/{main_experiment}/{metric}/{model}/"
        )
        ...
        if survival:
            ...
            directory_path = (
                f"{base_path}/{data_modality}/{experiment}/{metric}/{model}/"
            )
...
def setup_age_cutoff(
    ...
    if original_results_directory_path is not None:
        original_results_directory_path = (
            f"{original_results_directory_path}agecutoff_{age_cutoff}"
        )
...
def _get_experiment_vars(data_instance, X, lancet_vars):
    ...
    experiment_vars = {
        ...
        "age_sex_lancet2024": df_utils.pull_columns_by_prefix(
            X,
            [
                f"21003-{data_instance}.0",
                "31-0.0",
        ...
        + lancet_vars,
        "demographics_and_lancet2024": df_utils.pull_columns_by_prefix(
            X,
            [
                f"21003-{data_instance}.0",
                "31-0.0",
        ...
        "modality_only": {
            ...
            "neuroimaging": pickle.load(
                open(f"{data_path}/neuroimaging/idp_variables.pkl", "rb")
            ),
    ...
    experiment_vars["demographics_and_modality"] = {
        "proteomics": experiment_vars["all_demographics"]
        + experiment_vars["modality_only"]["proteomics"],
        "neuroimaging": experiment_vars["all_demographics"]
        + experiment_vars["modality_only"]["neuroimaging"],
        "cognitive_tests": experiment_vars["all_demographics"]
        + experiment_vars["modality_only"]["cognitive_tests"],
    }
    experiment_vars["demographics_modality_lancet2024"] = {
        "proteomics": experiment_vars["demographics_and_modality"]["proteomics"]
        + lancet_vars,
        "neuroimaging": experiment_vars["demographics_and_modality"]["neuroimaging"]
        + lancet_vars,
        "cognitive_tests": experiment_vars["demographics_and_modality"][
            "cognitive_tests"
        ]
        + lancet_vars,
    }
    experiment_vars["fs_modality_only"] = experiment_vars["modality_only"]
    experiment_vars["fs_demographics_and_modality"] = experiment_vars[
        "demographics_and_modality"
    ]
    experiment_vars["fs_demographics_modality_lancet2024"] = experiment_vars[
...
def subset_experiment_vars(
    data_modality, data_instance, experiment, X, lancet_vars, survival=False
):
    """
...
def _flaml_time_budgets():
    ...
    time_budgets = {
        "age_only": {
            "proteomics": {"lgbm": 10, "lrl1": 10},
            "neuroimaging": {"lgbm": 10, "lrl1": 10},
            "cognitive_tests": {"lgbm": 30, "lrl1": 100},
        },
        "all_demographics": {
            "proteomics": {"lgbm": 25, "lrl1": 100},
            "neuroimaging": {"lgbm": 30, "lrl1": 500},
            "cognitive_tests": {"lgbm": 150, "lrl1": 2500},
        },
        "age_sex_lancet2024": {
            "proteomics": {"lgbm": 75, "lrl1": 500},
            "neuroimaging": {"lgbm": 75, "lrl1": 600},
            "cognitive_tests": {"lgbm": 325, "lrl1": 3000},
        },
        "demographics_and_lancet2024": {
            "proteomics": {"lgbm": 100, "lrl1": 800},
            "neuroimaging": {"lgbm": 100, "lrl1": 800},
            "cognitive_tests": {"lgbm": 350, "lrl1": 3500},
        },
        "modality_only": {
            "proteomics": {"lgbm": 8500, "lrl1": 35000},
            "neuroimaging": {"lgbm": 9500, "lrl1": 36000},
            "cognitive_tests": {"lgbm": 350, "lrl1": 4500},
        },
        "demographics_and_modality": {
            "proteomics": {"lgbm": 9000, "lrl1": 35000},
            "neuroimaging": {"lgbm": 14000, "lrl1": 38000},
            "cognitive_tests": {"lgbm": 400, "lrl1": 5500},
        },
        "demographics_modality_lancet2024": {
            "proteomics": {"lgbm": 9500, "lrl1": 35000},
            "neuroimaging": {"lgbm": 14500, "lrl1": 40000},
            "cognitive_tests": {"lgbm": 450, "lrl1": 6500},
        },
        "fs_modality_only": {
            "proteomics": {"lgbm": 8500, "lrl1": 35000},
            "neuroimaging": {"lgbm": 9500, "lrl1": 36000},
...
def continuous_vars_for_scaling(
    ...
    continuous_cols = {
        "age_only": [f"21003-{data_instance}.0"],
        "all_demographics": df_utils.pull_columns_by_prefix(
            X, [f"21003-{data_instance}.0", "845-0.0"]
        ).columns.tolist(),
        "age_sex_lancet2024": df_utils.pull_columns_by_prefix(
            X, [f"21003-{data_instance}.0", "845-0.0"]
        ).columns.tolist()
    ...
    continuous_cols["demographics_and_modality"] = {
        "proteomics": continuous_cols["all_demographics"]
        + continuous_cols["modality_only"]["proteomics"],
        "neuroimaging": continuous_cols["all_demographics"]
        + continuous_cols["modality_only"]["neuroimaging"],
        "cognitive_tests": continuous_cols["all_demographics"]
        + continuous_cols["modality_only"]["cognitive_tests"],
    }
    continuous_cols["demographics_modality_lancet2024"] = {
        "proteomics": continuous_cols["demographics_and_modality"]["proteomics"]
        + continuous_lancet_vars,
        "neuroimaging": continuous_cols["demographics_and_modality"]["neuroimaging"]
        + continuous_lancet_vars,
        "cognitive_tests": continuous_cols["demographics_and_modality"][
            "cognitive_tests"
        ]
        + continuous_lancet_vars,
    }
    ...
    continuous_cols["fs_demographics_and_modality"] = continuous_cols[
        "demographics_and_modality"
    ]
    continuous_cols["fs_demographics_modality_lancet2024"] = continuous_cols[
        "demographics_modality_lancet2024"
    ]
...
def save_feature_importance(automl, directory_path, region_index):
    ...
    fi_df.to_parquet(
        f"{directory_path}/feature_importance_region_{region_index}.parquet"
    )
...
def iterative_fs_inference(
    ...
    for j, tf in enumerate(top_feature_names[:100]):
        ...
        current_time = datetime.now().time()
        print(
            f"Done fitting model for region {region_index+1} with top {j+1} variables. {current_time}"
        )
...
def main():
    ...
    if age_cutoff is not None:
        directory_path, original_results_directory_path, X, y, region_indices = (
            setup_age_cutoff(
                directory_path,
                original_results_directory_path,
                X,
    ...
    print(
        f"Running {experiment} experiment, region {region_index}, autoML time budget of {time_budget} seconds, {metric} as the metric, and an age cutoff of {age_cutoff} years"
    )
    ...
    X_train, y_train, X_test, y_test, region = subset_train_test_data(
        X, y, data_modality, region_index, region_indices
    )
    if model == "lrl1":
    ...
    if "fs_" in experiment:
        automl_settings["log_file_name"] = (
            f"{original_results_directory_path}/results_log_{region_index}.json"
        )
        ...
        (
            ...
            filename=f"{original_results_directory_path}/results_log_{region_index}.json",
            time_budget=time_budget,
        )
        ...
        automl_settings["log_file_name"] = (
            f"{directory_path}/results_log_{region_index}.json"
        )
